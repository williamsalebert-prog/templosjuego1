...
const imagenesPiezas = {};   // tipo -> Image
const colorBordeEquipo = ['#CC0000', '#1E3A8A'];

// Precarga imágenes para todos los tipos de pieza registrados
function precargarImagenes() {
    const extensiones = ['.jpg', '.jpeg', '.png'];
    for (let tipo of piezasRegistradas.keys()) {
        if (imagenesPiezas[tipo]) continue; // ya cargada
        let cargada = false;
...
// Copia del tablero usando el registro de piezas
...
function iniciarJuego() {
    ...
    // Colocación inicial (solo F1, pero podrías poner cualquier tipo)
    board[7][3] = new F1(0);
    board[6][2] = new F1(0);
    board[8][2] = new F1(0);
    board[7][19] = new F1(1);
